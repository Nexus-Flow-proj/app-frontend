...
### shadcn/ui is mandatory for interactive elements — never use native equivalents

Before writing or editing any component, check whether a shadcn/ui primitive exists for it. Never use a raw native element when a shadcn equivalent is available in this project.
...
| `<select>` / `<option>`         | `Select`, `SelectTrigger`, `SelectValue`, `SelectContent`, `SelectItem` — `@/components/ui/select`     |
| native dropdown menus           | `DropdownMenu`, `DropdownMenuTrigger`, `DropdownMenuContent`, `DropdownMenuItem`, `DropdownMenuCheckboxItem`, `DropdownMenuSeparator`, `DropdownMenuLabel` — `@/components/ui/dropdown-menu` |
| modal / dialog `<div>`s         | `Dialog` (centered) or `Sheet` (slide-out drawer) — `@/components/ui/dialog` / `@/components/ui/sheet` |
| `<hr>` / manual divider divs    | `Separator` — `@/components/ui/separator`                                                              |
| badge/pill `<span>`             | `Badge` — `@/components/ui/badge`                                                                      |
| loading placeholder divs        | `Skeleton` — `@/components/ui/skeleton`                                                                 |
| hover tooltips                  | `Tooltip`, `TooltipTrigger`, `TooltipContent` — `@/components/ui/tooltip`                              |
| avatar `<img>`/initials div     | `Avatar`, `AvatarImage`, `AvatarFallback` — `@/components/ui/avatar`                                    |
...
| progress bar div                | `Progress` — `@/components/ui/progress`                                                                 |
| card container div              | `Card`, `CardHeader`, `CardContent`, `CardFooter` — `@/components/ui/card` (only for true card layouts, not every bordered div) |

**Exceptions — native elements remain correct here:**

- `<div>` for layout/flex/grid containers — shadcn has no layout primitive.
- `<input type="date">` — shadcn has no native date input. Keep it, but style it with the same Tailwind classes as `Input` (`border-input`, `bg-background`, `focus-visible:ring-ring`) until a date picker (`Popover` + `Calendar`) is explicitly requested.
- `<span>`, `<p>`, `<h1>`–`<h6>` for typography — not interactive elements.
- `<svg>` for icons not covered by `lucide-react`.

**Rule of thumb:** if it's clickable, typeable, toggleable, selectable, or shows a floating layer (menu/tooltip/dialog), it must be a shadcn component — never a native tag with Tailwind classes bolted on to imitate one. If unsure whether a shadcn component exists for a given pattern, ask before defaulting to a native element.

When editing an existing file that still uses native elements for these patterns, flag it and ask whether to migrate it in the same pass or leave it for a separate PR.
