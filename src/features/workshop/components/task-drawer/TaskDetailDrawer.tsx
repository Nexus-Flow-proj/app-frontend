...
import { Calendar, Flag, Layers3, StickyNote, Trash2, User } from "lucide-react";
...
export function TaskDetailDrawer({ objectId, onClose }: TaskDetailDrawerProps) {
  ...
  const obj = objectId ? (objects.find((o) => o.id === objectId) ?? null) : null;
  ...
  return (
    <Sheet open={!!objectId} onOpenChange={(open) => !open && onClose()}>
      <SheetContent side="right" className="flex w-[420px] flex-col gap-0 p-0 sm:w-[420px]">
        <SheetHeader className="border-b border-border px-6 py-4">
          <div className="flex items-center gap-2">
            {icon}
            <span className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {obj?.type === CanvasObjectType.SECTION_FRAME ? "Section" : form.kind}
            </span>
          </div>
        ...
        <div className="flex flex-1 flex-col gap-5 overflow-y-auto px-6 py-5">
          ...
          {!isSticky && (
            <div className="flex flex-col gap-1.5">
              ...
              <Input value={form.title} onChange={(e) => setValue("title", e.target.value)} placeholder="Item title" />
            </div>
          )}
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <Select value={form.status} onValueChange={(value) => setValue("status", value as TaskStatus)}>
                    <SelectTrigger className="h-8 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <Select value={form.priority} onValueChange={(value) => setValue("priority", value as TaskPriority)}>
                    <SelectTrigger className="h-8 text-xs">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="flex gap-2">
                <span className="inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[11px] font-medium" style={{ background: statusCfg.bg, color: statusCfg.text }}>
                  <span className="h-1.5 w-1.5 rounded-full" style={{ background: statusCfg.dot }} />
                  {statusCfg.label}
                </span>
                <span className="inline-flex items-center gap-1.5 rounded-full px-2 py-1 text-[11px] font-medium" style={{ background: priorityCfg.bg, color: priorityCfg.text }}>
        ...
        <div className="flex items-center justify-between border-t border-border px-6 py-3">
          <Button variant="ghost" size="sm" className="gap-1.5 text-xs text-destructive hover:text-destructive" onClick={handleDelete}>
